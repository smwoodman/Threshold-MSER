...
int fillTopBottom(cv::Mat& img, int top, int bottom, int fill) {
    if (top + bottom > img.rows) {
        std::cerr << "Error: The size of the top and bottom crops are larger then the image." << std::endl;
        return 1;
    }
    
    // Fill the sides with pixel "fill"
    for (int i = 0; i < top; i++) {
        img.row(i).setTo(fill);
    }
    for (int i = img.rows - bottom; i < img.rows; i++) {
        img.row(i).setTo(fill);
    }
