...
void helpMsg(std::string executable, Options options) {
	...
	<< std::left << std::setw(30) << "  -VV, --verbosePlus"<<" Enable VerbosePlus Mode [More VERBOSE] (Default: "<<options.verboseModePlus<<")\n"
        ...
        << std::left << std::setw(30) << "  -t, --top-crop" << "Crop this many pixels off of the top of the image\n"
        << std::left << std::setw(30) << "  -b, --bottom-crop" << "Crop this many pixels off of the bottom of the image\n"
...
int main(int argc, char **argv) {
	...
	while (i < argc) {
		...
		if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
			...
			return 0;
        		...
        		// Validate the input type
		...
		else if (strcmp(argv[i], "-t") == 0 || strcmp(argv[i], "--top-crop") == 0) {
            		// Validate the input type
            		if ( !isInt(argv[i+1]) ) {
                		std::cerr << argv[i+1] << " is not a valid input. Top crop must be a positive integer." << std::endl;
                		return 1;
            		} 
            		...
            		options.top = std::stoi(argv[i+1]);
            		i+=2;
		} 
		else if (strcmp(argv[i], "-b") == 0 || strcmp(argv[i], "--bottom-crop") == 0) {
            		// Validate the input type
            		if ( !isInt(argv[i+1]) ) {
                		std::cerr << argv[i+1] << " is not a valid input. Bottom crop must be a positive integer." << std::endl;
                		return 1;
            		} 
			else if (std::stoi(argv[i+1]) < 0) {
                		std::cerr << argv[i+1] << " is not a valid input. Bottom crop must be a positive integer." << std::endl;
            		...
            		options.bottom = std::stoi(argv[i+1]);
            		i+=2;
		} 
    ...
    if (options.verboseMode) {
		...
		std::cout<<"top            : "<<options.top<<"\n"<<std::endl;
		std::cout<<"bottm          : "<<options.bottom<<"\n"<<std::endl;
		...
		std::string segmentDir = options.outputDirectory + "/" + fileName;
 		...
 		if (options.verboseMode) {
		    std::cout<<"Dir: "<<imgDir<<" Created\n"<<std::endl;
                ...
                int fill_tb = fillTopBottom(imgGray, options.top, options.bottom);
                if (fill_tb != 0) {
                    exit( 1 );
                }

                int fill_lr = fillSides(imgGray, options.left, options.right);
                if (fill_lr != 0) {
                    exit( 1 );
                }
